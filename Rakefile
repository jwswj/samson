...
task :test do
  group = if ENV["BUILDKITE_PARALLEL_JOB"]
    # Buildkite groups start at 0
    ENV["BUILDKITE_PARALLEL_JOB"].to_i + 1
  else
    1
  end

  groups = if ENV["BUILDKITE_PARALLEL_JOB_COUNT"]
    ENV["BUILDKITE_PARALLEL_JOB_COUNT"].to_i
  else
    1
  end
